----------------------------------------------------------------
-- Shapes and Colors :: 
-- Part 6: Displaying dynamic information using parameter watching
-- Goal: Display the name of the current object and material diffuse color at runtime
----------------------------------------------------------------
...
function setup()
    ...
    -- Add function to display active shape info
    function getActiveShapeInfo()
        if Shape >= 1 and Shape <= #shapes then
            local shapeName = shapeData[Shape].name
            local currentColor = shapes[Shape]:get(craft.renderer).material.diffuse
            return shapeName .. " - Diffuse Color: " .. tostring(currentColor)
        else
            return "No shape selected"
        end
    end
    parameter.watch("getActiveShapeInfo()")
